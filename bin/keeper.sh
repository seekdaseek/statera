...
# takes its own lockfile, because macOS has no flock(1) and a double post costs
# real OKB. On the VPS this is the line to install:
#
#   */5 * * * * /opt/statera/bin/keeper.sh --post >> /opt/statera/cron.log 2>&1
#
...
LOCK="${STATERA_LOCK_FILE:-/var/lock/statera-keeper.lock}"
